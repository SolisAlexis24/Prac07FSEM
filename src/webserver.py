...
class WebServer(BaseHTTPRequestHandler):
	...
	def _parse_post(self, json_obj):
		if not 'action' in json_obj or not 'delay' in json_obj:
			return
		switcher = {
			'update_speed' : LCDHandler.updateSpeed,
			'update_units' : LCDHandler.updateUnits
		}

		func = switcher.get(json_obj['action'], None)
		if func:
			if json_obj['action'] == 'update_speed':
				delay = float(json_obj.get('delay', 0.25))
				print('\tCall {}(delay={})'.format(func, delay))
				func(delay)
			elif json_obj['action'] == 'update_units':
				CelsiusState = json_obj.get('celsius')
				FahrenheitState = json_obj.get('fahrenheit')
				print('\tCall {}(C={}, F={})'.format(func, CelsiusState, FahrenheitState))
				func(CelsiusState, FahrenheitState)
